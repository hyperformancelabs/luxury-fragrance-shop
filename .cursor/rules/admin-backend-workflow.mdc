...
5. **API Testing:** First, call `localhost:8080/api/v1/auth/emp/login` api to employee `vhphat0807` with password `Admin@123`. Then use terminal tool call to execute `curl` commands with `/api/v1` prefix directly on cursor integrated terminal, e.g.:
   ```bash
